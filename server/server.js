...
Meteor.methods({
	...
	"updateUserAccount": function(userId, options) {
		...
		if(options.roles == "salesman" || options.roles == "s_manager") {
			var repOptions = {};
			if(userId) repOptions._id = userId;
			if(options.profile.name) repOptions.name = options.profile.name;
			if(options.profile.email) repOptions.email = options.profile.email;
			if(!Reps.findOne( { _id: userId } ) ) Reps.insert(repOptions);
		}
